...
class Manager extends AssetManager
{
    ...
    public function __construct($di)
    {
        ...
        $this->_config = ArrayUtils::array_merge_recursive_distinct($this->_config, $this->getDI()->getShared('config')->application->assets->toArray());
    ...
    public function outputCss($collectionName = self::DEFAULT_COLLECTION_CSS)
    {
        ...
        if ($collection->getJoin()) {
            ...
            $filepath = self::GENERATED_STORAGE_PATH . $filename = $filename = $this->getCollectionFileName($collection, self::FILENAME_PATTERN_CSS);
            $collection
